...
totalRejected = 0;
  ...
  get placementRate(): string {
    const total = this.totalSelected + this.totalRejected + this.totalPending;
    if (total === 0) return '0%';
    return ((this.totalSelected / total) * 100).toFixed(1) + '%';
  }
  ...
  loadPlacementStatusCounts() {
    ...
    this.api.getApplications().subscribe(apps => {
      ...
      this.totalPending = activeApps.filter(app => app.status?.toUpperCase() === 'PENDING').length;

  
      this.totalSelected = activeApps.filter(app => app.status?.toUpperCase() === 'SELECTED').length;
  
      this.totalRejected = activeApps.filter(app => app.status?.toUpperCase() === 'REJECTED').length;
    }, error => {
      console.error('Failed to load placement status counts', error);
    });
  }
